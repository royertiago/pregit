/* Classe base da lista duplamente encadeada. O elemento duplamente 
 * encadeado aponta para dois outros elementos: o próximo e o anterior.
 * Não armazena dados; suas classes derivadas implementarão esta
 * funcionalidade.
 *
 * Por serem implementações internas, todos os métodos de 
 * inserção/leitura/remoção assumem que a lista "não tem fim"
 * (o fim não será atingido no processo de inserção/leitura/remoção),
 * e atuam com posicionamento relativo à posição atual.
 *
 * Em todos os métodos, caso não seja possível extrair o dado
 * armazenado em algum elemento duplamente encadeado, assumiremos que
 * a lista terminou-se.
 *
 * Devido a erros no planejamento inicial, algumas funções de inserção 
 * e remoção trabalham com elementos duplamente encadeados, e outras, 
 * com ponteiros para T.
 */
...
class ElementoDE {
    ...
    /* Quem extender esta classe é responsável por excluir os próximos
     * elementos. */
...
private:
     ...
     * Caso não haja dado armazenado, false é retornado e dado não é 
     * alterado. */
     ...
     * (pos = 0 implica que o elemento será inserido logo após o 
     * elemento atual) */
    void _adicionarPosicao( ElementoDE<T>* elemento, int pos );

    /* Armazena em elemento um ponteiro para o elemento encadeado que 
     * está pos posições à frente, e o remove da lista. */
    void _retiraPosicao( ElementoDE<T>*& elemento, int pos );

    /* Retira a primeira ocorrência de dado da lista, e retorna true. 
     * Um ponteiro para a primeira ocorrência é posto em retorno. 
     * Caso não seja encontrado, false é retornado e retorno
     * não é alterado.
     *
     * Este método supõe sobrecarga do operador == para o tipo T.
     *
     * Específicamente para este método, a busca é realizada no próximo
     * elemento, e não neste; então, pode haver a necessidade de ser
     * feito o teste externo para o elemento inicial. */
    bool _retiraEspecifico( const T* dado, T*& retorno );

    /* Retorna quantas posicões à frente da lista está o dado, ou -1 
     * caso não esteja.
     *
     * Específicamente para este método, a busca é realizada no próximo
     * elemento, e não neste;
     * então, pode haver a necessidade de ser feito o teste externo
     * para o elemento inicial. */
    int _posicao( const T* dado ) const;

    /* Armazena em elemento o elemento encadeado pos posições à frente
     * deste.
     * Retorna false caso a posição não exista; neste caso, elemento
     * não é alterado.
     * Note que a lista é indexada em zero; zero posições à frente é
     * esta posição aqui. */
    void _lerPosicao( ElementoDE<T>*& elemento, int pos );
    void _lerPosicao( const ElementoDE<T>*& elemento, int pos ) const;

    /* Informação de controle; apenas o sentinela deve ser o último
     * nodo. Neste caso, o destrutor do objeto anterior não deve
     * encadear destrutores. */
...
template <typename T>
void ElementoDE<T>::_adicionarPosicao(
                ElementoDE<T>* elemento, int pos ) {
    if( pos == 0 ) {
...
template <typename T>
void ElementoDE<T>::_retiraPosicao( 
                ElementoDE<T>*& elemento, int pos ) {
    if( pos == 0 ) {
...
bool ElementoDE<T>::_retiraEspecifico( const T* dado, T*& retorno ) {
    ...
    if( (*d) == (*dado) ) {
        ...
        /* Não queremos que a deleção do elemento encadeie destrutores,
         * apenas desaloque memória. */
        operator delete( del );
        return true;
    }
...
template <typename T>
void ElementoDE<T>::_lerPosicao(
                const ElementoDE<T>*& elem, int pos ) const {
    if (pos == 0)
        elem = this;
    else if (pos > 0)
        this->_proximo->_lerPosicao( elem, pos - 1 );
