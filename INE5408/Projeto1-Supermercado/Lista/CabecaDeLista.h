 ...
 *
 * O construtor de cópia e o operador de atribuição usam do construtor de cópia do tipo T.
 *
 * O operador de atribuição de transferência e construtor de transferência "zeram" o estado
 * do objeto fonte; isto é, esvaziam o fonte, e este fica como que recém-construído.
 */
...
public:
    ...
    /* Limpa esta cabeça de lista e copia todos os elementos da cabeça passada.
     * Vale-se do construtor de cópia do tipo T. */
    CabecaDeLista& operator=( const CabecaDeLista<T>& );

    /* Operador de atribuição de transferência.
     * O objeto fonte terminará como se estivesse recém-construído.
     * O objeto destino destruirá todos os elementos que estão armazenados. */
     ...
     * e false caso contrário. Um ponteiro para o dado encontrado é posto em retorno;
     * caso o dado não seja encontrado, retorno não é alterado.
     *
     * Esta função assume que o operador == está sobrecarregado para os objetos da classe T. 
     *
     * O elemento encontrado não é destruído, apenas perde-se a referência a ele na lista;
     * caso queira destruí-lo, use o método destroiEspecífico */
    bool retiraEspecifico( const T& dado, T*& retorno );
    bool retiraEspecifico( const T* dado, T*& retorno );

    /* Remove a primeira ocorrência de dado da lista e retorna true, caso ele seja encontrado,
     * e false caso contrário. A ocorrência encontrada é destruída.
     *
     * Essa função assume que o operador == está sobrecarregado para os objetos da classe T.
     *
     * Note que a ocorrência encontrada é destruída; caso queira preservá-la, use o método
     * retiraEspecifico. */
    ...
    // Retorna a posição da primeira ocorrência de dado na lista, ou -1 caso não seja encontrado.
    ...
    // Retorna true caso todos os elementos sejam iguais, e na mesma ordem.
...
template <typename T>
CabecaDeLista<T>& CabecaDeLista<T>::operator=( const CabecaDeLista<T>& l ) {
    this->limpar();
    for( IteradorConstante<T> it = l.iterador(); !it.volta(); ++it )
        this->adiciona( new T(*it) );
}
...
template <typename T>
CabecaDeLista<T>::~CabecaDeLista() {
    if ( !this->vazia() ) // Se ela estiver vazia, this->_proximo == this. 
        delete this->_proximo;
}
...
bool CabecaDeLista<T>::adicionaPosicao( T* dado, int pos ) {
    ...
    this->_adicionarPosicao( elem, pos ); // Função herdada de ElementoDE.
    tam++;
    return true;
}
 ...
 * Note que a lista é indexada em zero.
 *
 * Para reduzir pela metade o tempo esperado de deleção, a função exclui "de trás para frente"
 * caso o elemento esteja depois da metade da lista. */
template <typename T>
 ...
 * Esta função assume que o operador == está sobrecarregado para os objetos da classe T. 
 *
 * O elemento encontrado não é destruído, apenas perde-se referência a ele na lista;
 * caso queira destruí-lo, use o método destroiEspecífico */
template <typename T>
inline bool CabecaDeLista<T>::retiraEspecifico( const T& dado, T*& retorno ) {
...
template <typename T>
bool CabecaDeLista<T>::operator==( const CabecaDeLista<T>& alce ) const {
